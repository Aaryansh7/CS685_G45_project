# program to add latitude and longitude and cyclcicty in datasheet
...
11.Annual precession of orbit per year, omega_yr (rad/yr) 
12.Perigee altitude, alt_p (km) 
13.Apogee altitude, alt_a (km) 
14.Average altitude, alt (km) 
15.Mean Local Time of Ascending Node, mltan (hour) 
'''
...
file = open('data_files/envisat.tle', 'r') 
...
for i in tqdm(range(s_count)):
	...
	semi_major_ = semi_major_axis(n)*1000 #in m
	...
	omega_yr = annual_precession_(inclination_, orbital_period_, semi_latus_rectum_)
	...
	mean_local_time_ascending_node_ = mean_time_ascending_node(inclination_, right_ascension_ascending_node_, epoch_)
	cycle = Search_Satellite_Repeat_Cycle_test. Repeat_Cycle(satname_,s,t,epoch_)
	...
	Days_.append(cycle['Days_']) 
	Min_Distance.append(cycle['Min_Distance'])


list_tuples = list(zip(satname, epoch, mean_motion, semi_major, orbital_period, eccentricity, semi_latus_rectum, inclination, right_ascension_ascending_node,
					  argument_perigee, annual_precession,  altitude_perigee, altitude_apogee, avg_altitude, mean_local_time_ascending_node,
					  Initial_Latitude_, Initial_Longitude_, Latitude_, Longitude_, Diff_Latitude, Diff_Longitude, Days_, Min_Distance))

dframe = pd.DataFrame(list_tuples, columns=['SATNAME', 'EPOCH', 'MEAN MOTION(rev/day)', 'SEMI MAJOR (m)', 'ORBITAL PERIOD(sec)', 'ECCENTRICITY', 'SEMI LATUS RECTUM(m)', 'INCLINATION(deg)',
											 'RIGHT ASCENSION ASCENDING NODE(deg)', 'ARGUMENT PERIGEE(deg)', 'ANNUAL PRECESSION(rad/yr)', 'ALTITUDE PERIGEE(km)', 
											 'ALTITUDE APOGEE(km)', 'AVERAGE ALTITUDE(km)', 'MEAN LOCAL TIME ASCENDING NODE(hour)',
...
print(dframe) 
dframe.to_csv('data_files/satellite_data+cyclicity.csv')
