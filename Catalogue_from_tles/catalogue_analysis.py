# This program makes the detalied catalogue from the tle data. Following our the data-fields that are added.
'''
1.Satname 
...
file = open('../data_files/recent_elset.tle', 'r') 
...
list_tuples = list(zip(norad_id, satname, epoch, mean_motion, semi_major, orbital_period, eccentricity, semi_latus_rectum, inclination, right_ascension_ascending_node,
					  argument_perigee,  altitude_perigee, altitude_apogee, avg_altitude, circular_orbit, orbit_classification, annual_precession, sun_synchronous,
					  mean_local_time_ascending_node, Days_, mean_anamoly, mean_longitude, sublongitude))
					  #Initial_Latitude_, Initial_Longitude_, Latitude_, Longitude_, Diff_Latitude, Diff_Longitude, Days_, Min_Distance))

dframe = pd.DataFrame(list_tuples, columns=['NORAD ID', 'SATNAME', 'EPOCH', 'MEAN MOTION(rev/day)', 'SEMI MAJOR (m)', 'ORBITAL PERIOD(sec)', 'ECCENTRICITY', 'SEMI LATUS RECTUM(m)', 'INCLINATION(deg)',
											 'RIGHT ASCENSION ASCENDING NODE(deg)', 'ARGUMENT PERIGEE(deg)', 'ALTITUDE PERIGEE(km)', 
											 'ALTITUDE APOGEE(km)', 'AVERAGE ALTITUDE(km)', 'CIRCULAR ORBIT', 'ORBIT CLASS', 'ANNUAL PRECESSION(rad/yr)',
											 'SUN-SYNCHRONOUS', 'MEAN LOCAL TIME ASCENDING NODE', 'DAYS', 'MEAN ANAMOLY(deg)', 'MEAN LONGITUDE(deg)', 'SUBLONGITUDE'])
...
#print(dframe) 
dframe.to_csv('../data_files/catalogue.csv')
